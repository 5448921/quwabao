	...
	$.ajax({
		...
		success:function(data){
			...
			$.each(data.users, function(i, m) {
				...
				if(m.sex==0){
					onlineZNodes.push({id : m.id,name : m.trueName,pId : m.deptId,
						icon:"resources/images/user/U11.png",href:"instantMessage/addPage.do?userId="+m.id+"&rel=xttx_jsgt_add",dwzTarget:"dialog",
						rel:"xttx_jsgt_add",dialogParam:{width:1000,height:450}
					});
				}else{
					onlineZNodes.push({id : m.id,name : m.trueName,pId : m.deptId,
						icon:"resources/images/user/U01.png",href:"instantMessage/addPage.do?userId="+m.id+"&rel=xttx_jsgt_add",dwzTarget:"dialog",
						rel:"xttx_jsgt_add",dialogParam:{width:1000,height:450}
					});
				}
